...
import type { BacklogItem, Task, TaskJournalEntry, WorkspaceFixture } from "./domain/model";
...
function WorkspaceViewer({
  ...
  const [taskJournalItems, setTaskJournalItems] = useState<TaskJournalEntry[]>([]);
  const [taskJournalLoading, setTaskJournalLoading] = useState(false);
  const [taskJournalError, setTaskJournalError] = useState<string>();
  ...
  useEffect(() => {
    ...
    setTaskJournalItems([]);
    setTaskJournalError(undefined);
    if (!selectedTask) {
      setTaskJournalLoading(false);
      return () => controller.abort();
    }
    const target = { workspaceId, taskId: selectedTask.id, taskPublicId: selectedTask.publicId };
    setTaskJournalLoading(true);
    traceViewer("task-journal:request", {
      userEvent: "task-selection",
      calculatedTarget: target,
      reactState: { selectedTaskId: selectedId, workspaceRevision: graph.workspace.revision },
      controllerState: "active",
    });
    void fetchTaskJournal(workspaceId, selectedTask.publicId, controller.signal).then((page) => {
      if (controller.signal.aborted) return;
      setTaskJournalItems(page.items);
      setTaskJournalLoading(false);
      traceViewer("task-journal:store-committed", {
        calculatedTarget: target,
        reactState: { journalCount: page.items.length, nextCursor: page.nextCursor },
        controllerState: "settled",
      });
      window.requestAnimationFrame(() => traceViewer("task-journal:dom-rendered", {
        calculatedTarget: target,
        renderedDom: {
          taskPublicId: document.querySelector<HTMLElement>("[data-task-journal]")?.dataset.taskJournal,
          entryCount: document.querySelectorAll("[data-task-journal-entry]").length,
        },
      }));
    }).catch((error: unknown) => {
      if (controller.signal.aborted) return;
      setTaskJournalLoading(false);
      setTaskJournalError(error instanceof Error ? error.message : "Task journal unavailable");
      traceViewer("task-journal:store-failed", {
        calculatedTarget: target,
        reactState: { error: error instanceof Error ? error.name : "unknown" },
        controllerState: "settled",
      });
    });
    return () => {
      controller.abort();
      traceViewer("task-journal:request-aborted", { calculatedTarget: target, controllerState: "aborted" });
    };
  }, [workspaceId, selectedTask?.id, selectedTask?.publicId, graph.workspace.revision]);
    ...
    <main className="app-shell" data-workspace-id={graph.workspace.id} data-layout-mode={renderedLayoutMode} data-auth-state="authenticated" data-role={membership.role}>
      ...
      <section className={`workspace ${inspectorOpen ? "with-inspector" : ""}`} style={{ "--inspector-width": `${inspectorWidth}px` } as React.CSSProperties}>
        ...
        {inspectorOpen && <div className="inspector-panel">
          ...
          <Inspector fixture={graph} task={selectedTask} backlog={selectedBacklog} gateId={selectedGate?.id} membership={membership} csrfToken={csrfToken} journalItems={taskJournalItems} journalLoading={taskJournalLoading} journalError={taskJournalError} onTaskConfirmed={(taskId, execution) => {
            setFixture((current) => current ? {
              ...current,
              workspace: { ...current.workspace, revision: execution.workspaceRevision },
              tasks: current.tasks.map((item) => item.id === taskId ? { ...item, status: "confirmed" } : item),
            } : current);
...
function Inspector({ fixture, task, backlog, gateId, membership, csrfToken, journalItems, journalLoading, journalError, onTaskConfirmed, onLane, onGate }: { fixture: WorkspaceFixture; task?: Task; backlog?: BacklogItem; gateId?: string; membership: WorkspaceMembership; csrfToken: string; journalItems: TaskJournalEntry[]; journalLoading: boolean; journalError?: string; onTaskConfirmed: (taskId: string, execution: import("./api/auth").CommandExecution) => void; onLane: (id: string) => void; onGate: (id: string) => void }) {
  if (backlog) {
