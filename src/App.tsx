    ...
    <main className="app-shell" data-workspace-id={graph.workspace.id} data-layout-mode={renderedLayoutMode} data-auth-state="authenticated" data-role={membership.role}>
      ...
      <section className={`workspace ${inspectorOpen ? "with-inspector" : ""}`} style={{ "--inspector-width": `${inspectorWidth}px` } as React.CSSProperties}>
        ...
        {inspectorOpen && <div className="inspector-panel">
          ...
          <Inspector fixture={graph} task={selectedTask} backlog={selectedBacklog} gateId={selectedGate?.id} journalItems={taskJournalItems} journalLoading={taskJournalLoading} journalError={taskJournalError} onLane={(id) => navigate({ kind: "lane", id })} onGate={(id) => navigate({ kind: "gate", id })} />
        </div>}
      </section>
...
function Inspector({ fixture, task, backlog, gateId, journalItems, journalLoading, journalError, onLane, onGate }: { fixture: WorkspaceFixture; task?: Task; backlog?: BacklogItem; gateId?: string; journalItems: TaskJournalEntry[]; journalLoading: boolean; journalError?: string; onLane: (id: string) => void; onGate: (id: string) => void }) {
  if (backlog) {
