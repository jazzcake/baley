...
import { Check, CircleDot, LockKeyhole, Play, Wrench } from "lucide-react";
import { useLayoutEffect, useRef } from "react";
import { traceViewer } from "../debug/viewer-trace";
import type { TaskStatus } from "../domain/model";

export type TaskNodeData = { publicId: number; title: string; status: TaskStatus; lane: string; laneColor: string; dimmed: boolean; external: boolean; lockedByGate: boolean; compact?: boolean };
...
export function TaskNode({ data, selected }: NodeProps) {
  ...
  return (
    <article ref={nodeRef} style={{ "--lane-color": value.laneColor } as React.CSSProperties} className={`task-node status-${value.status} ${value.compact ? "compact" : ""} ${selected ? "selected" : ""} ${value.dimmed ? "dimmed" : ""} ${value.external ? "external" : ""}`}>
      <Handle type="target" position={Position.Left} />
      {value.lockedByGate && <span className="task-gate-lock" aria-label="Locked by gate"><LockKeyhole size={12} strokeWidth={2.4} /></span>}
      ...
      {!value.compact && <span className="task-status"><Icon size={12} />{value.status === "implemented" ? "완료확인 대기" : value.status}</span>}
