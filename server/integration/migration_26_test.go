...
func TestMigration26TaskJournalUpDownAndAppendOnly(t *testing.T) {
	...
	// Migration 30 owns conversational Task discard, migration 29 owns append-only
	// Events, migration 28 owns conversational evidence, and migration 27 is
	// forward-only. Move all four version markers
	// down first, then
	...
	t.Cleanup(func() {
		if err := postgres.Migrate(url, migrations, "up"); err != nil {
			t.Errorf("restore migrations 26 through 30: %v", err)
		}
	})
