package integration

import (
	"context"
	"os"
	"testing"

	"github.com/jazzcake/baley/server/internal/persistence/postgres"
)

func TestRepositoryReadinessAgainstPostgres(t *testing.T) {
	url := os.Getenv("BALEY_TEST_DATABASE_URL")
	if url == "" {
		t.Skip("BALEY_TEST_DATABASE_URL is not set")
	}
	t.Setenv("BALEY_LEASE_TOKEN_SECRET", "readiness-integration-secret")
	repo, err := postgres.Open(context.Background(), url)
	if err != nil {
		t.Fatal(err)
	}
	defer repo.Pool.Close()

	version, err := repo.Readiness(context.Background(), 30)
	if err != nil || version != 30 {
		t.Fatalf("Readiness() version=%d err=%v", version, err)
	}
	if _, err = repo.Readiness(context.Background(), 26); err == nil {
		t.Fatal("Readiness accepted an unexpected migration version")
	}
	if _, err = repo.Readiness(context.Background(), 27); err == nil {
		t.Fatal("Readiness accepted the pre-conversational-evidence schema")
	}
	if _, err = repo.Readiness(context.Background(), 28); err == nil {
		t.Fatal("Readiness accepted mutable Event history schema")
	}
	if _, err = repo.Readiness(context.Background(), 29); err == nil {
		t.Fatal("Readiness accepted the pre-conversational-discard schema")
	}
}
