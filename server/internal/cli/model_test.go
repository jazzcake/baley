...
func TestParseGoldenQueriesAndMutations(t *testing.T) {
	tests := []struct {
		...
		{[]string{"commit", "verify-remote", "commit-a", "--workspace", "workspace", "--remote-ref", "refs/heads/feature", "--actor", "agent", "--idempotency", "verify-commit"}, MutationInvocation, "commit.verify_remote", `{"commitId":"commit-a","remoteRef":"refs/heads/feature","workspaceId":"workspace"}`, false},
