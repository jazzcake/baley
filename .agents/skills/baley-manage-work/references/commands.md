...
```text
{
  ...
  decisionEvidence?: {
    ...
    action: "task.confirm" | "task.discard", taskId, workspaceRevision, commandHash
  }
}
```

Use `/v1/commands/preview` for a write-free evaluation and `/v1/commands/execute` for mutation. Ordinary `task.confirm` and `task.discard` carry exact typed `decisionEvidence` from the user's explicit conversation statement. In an established exact-Task context, action-only discard wording such as `삭제합시다` is explicit; questions, negations, conditions, and vague agreement are not. Baley derives the human approver from the authenticated MCP gateway's linked account, revalidates current capability, and binds/consumes the evidence for the exact action, target, Workspace revision, canonical command hash, idempotency key and Agent executor. Never supply an approver Actor ID or request a Viewer grant for either Task decision.

An explicit `all_awaiting_confirmation` statement may cover multiple eligible Tasks, but it is not a batch mutation. Each Task requires a fresh preview, caller-generated unique UUID decision ID, and current revision. The decision ID is not a provider thread/turn/message ID; `conversationRef` is an opaque non-secret audit reference, not an externally authenticated conversation binding. For selected Tasks, the Agent interprets the user's natural-language statement and established Task context, preserves the verbatim statement, and declares each target with `task` scope and its assigned action. The server binds that typed declaration without requiring the human to use a comma-delimited command grammar. Other human-only commands retain browser-session grants where their contract requires one.
