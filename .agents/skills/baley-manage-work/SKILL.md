...
The human may make a Task confirmation or discard decision explicitly in the conversation. For ordinary `task.confirm` and `task.discard`, fresh-preview the exact command, create a target-unique UUID decision ID, and execute through MCP with typed `decisionEvidence`: source `conversation`, a stable non-secret opaque conversation reference, the verbatim decision statement, scope `task` (or `all_awaiting_confirmation` for confirmation only), the exact action, target Task public ID, Workspace revision, and canonical command hash. In an established exact-Task context, an action-only discard statement such as `삭제합시다`, `폐기해 주세요`, or `discard` is explicit enough; questions, negations, conditions, vague agreement, and target-changing language are not. The decision ID is generated for the evidence record; do not reuse a provider thread, turn, or `msg_...` identifier as that ID. Baley records the conversation reference but does not authenticate it against the external conversation provider. Never infer approval or discard from tone, implementation status, silence, or an ambiguous statement.
...
When one explicit statement assigns different actions to numbered Tasks, preserve the verbatim statement and use scope `task` for every target. The supported conservative form is comma-delimited `#<id>` groups whose final item carries `confirm|complete|확인|완료|discard|delete|remove|폐기|삭제`, for example `#61, #62 confirm, #63 폐기, #64, #65도 삭제`. Sequentially execute `task.confirm` or `task.discard` only for IDs assigned to that action, with a fresh preview, current revision, and unique decision ID each time. Do not label such a selected-target statement `all_awaiting_confirmation`, and do not ask for a Viewer grant.
