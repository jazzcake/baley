...
10. On the compact MCP profile, use the exact `baley_task_rework_preview` and `baley_task_rework_execute` tools for the common implemented-to-in_progress correction workflow. For other mutations, use `baley_command_catalog` only when command discovery is needed, preview with `baley_command_preview`, execute routine Operator commands with `baley_command_execute`, and route human or conditionally approved commands through `baley_command_execute_with_approval` only after any required fresh browser grant is available. Read normal revision/context through `baley_workspace_context` or `baley_task_get`; `baley_workspace_get` and the remaining legacy per-command typed tools stay in the explicit full profile for diagnostics or rare administration.
