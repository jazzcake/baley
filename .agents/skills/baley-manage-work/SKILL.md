...
8. Validate obvious invariants before preview:
   ...
   - `task.confirm` and `task.discard` have explicit current-conversation decision evidence for the exact Task; confirmation also supports an explicit all-awaiting-confirmation scope. Other human-only boundaries have their applicable fresh browser-session grant.
...
10. On the compact MCP profile, use `baley_command_catalog` only when command discovery is needed, preview with `baley_command_preview`, execute routine Operator commands with `baley_command_execute`, and route human or conditionally approved commands through the compatibility-stable `baley_command_execute_with_approval`. For `task.confirm` and `task.discard`, attach the exact typed `decisionEvidence` from the user's explicit current-conversation decision; never infer it or request a Viewer grant. Other human-only boundaries continue to use a fresh browser grant where specified. The explicit full profile may expose per-command typed tools for diagnostics or rare administration.
...
The human may make a Task confirmation or discard decision explicitly in the conversation. Interpret the user's ordinary language and established exact Task context; never make the human restate that decision in an internal command grammar. For ordinary `task.confirm` and `task.discard`, fresh-preview the exact command, create a target-unique UUID decision ID, and execute through MCP with typed `decisionEvidence`: source `conversation`, a stable non-secret opaque conversation reference, the verbatim decision statement, scope `task` (or `all_awaiting_confirmation` for confirmation only), the exact action, target Task public ID, Workspace revision, and canonical command hash. The Agent declares that typed meaning; the server binds and audits it without reparsing the prose. In an established exact-Task context, an action-only statement such as `삭제합시다`, `폐기해 주세요`, `confirm`, or `discard` is explicit enough. Questions, negations, conditions, vague agreement, and target-changing language are not decisions. The decision ID is generated for the evidence record; do not reuse a provider thread, turn, or `msg_...` identifier as that ID. Baley records the conversation reference but does not authenticate it against the external conversation provider. Never infer approval or discard from tone, implementation status, silence, or an ambiguous statement.
...
When one explicit statement covers several Tasks or assigns different actions, interpret the targets and actions from the natural-language statement plus established workflow context. Preserve the verbatim statement and use scope `task` for every selected target. Wording such as `이제 #61, #62 삭제해줘`, `둘 다 confirm`, or a mixed Korean/English sentence does not need normalization by the human. Sequentially execute only the action the human assigned to each target, with a fresh preview, current revision, and unique decision ID each time. Do not label a selected-target statement `all_awaiting_confirmation`, and do not ask for a Viewer grant.
...
Apply the same outcome-first approach to human-only Gate, Lane, and Workspace decisions: describe the real-world effect and decision evidence first, while preserving exact revision/hash/snapshot binding internally. `task.discard` instead uses the explicit conversational decision path above. `task.rework` is an Agent Operator action and does not require human approval. V1 has no persisted approval inbox.
