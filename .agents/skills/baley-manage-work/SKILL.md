...
The human may make a Task confirmation decision explicitly in the conversation. For ordinary `task.confirm`, fresh-preview the exact command, create a target-unique UUID decision ID, and execute through MCP with typed `decisionEvidence`: source `conversation`, a stable non-secret opaque conversation reference, the verbatim decision statement, scope `task` or `all_awaiting_confirmation`, action `task.confirm`, target Task public ID, Workspace revision, and canonical command hash. The decision ID is generated for the evidence record; do not reuse a provider thread, turn, or `msg_...` identifier as that ID. Baley records the conversation reference but does not authenticate it against the external conversation provider. Never infer approval from tone, implementation status, silence, or an ambiguous statement.
...
When one explicit statement assigns different actions to numbered Tasks, preserve the verbatim statement and use scope `task` for each confirmation target. The supported conservative form is comma-delimited `#<id>` groups whose final item carries `confirm|complete|확인|완료|discard|폐기`, for example `#61, #62 confirm, #63 폐기, #64, #65도 폐기`. Execute `task.confirm` only for IDs assigned to a confirmation action. Do not label such a selected-target statement `all_awaiting_confirmation`; other actions still follow their own human-only transport contract.
