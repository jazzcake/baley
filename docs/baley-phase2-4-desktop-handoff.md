...
- PostgreSQL과 MCP integration은 환경 변수 미설정으로 skip됐으므로 통합 완료로 판정하지 않음
- 실제 Git repository, HTTP/API, CLI, project init과 Viewer 연결도 데스크탑 검증 대기
...
명시적으로 실행되지 않은 두 integration test:
...
성공 조건은 MCP test가 skip 없이 14개 기존 Gate vertical-slice tool을 확인하고 graph query를 완료하는 것이다.
...
1. Run schema와 Repository port
2. `run.start` + pending Task 자동 시작 transaction
