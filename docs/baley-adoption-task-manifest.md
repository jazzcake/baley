...
| AE-AUTH-03 | `embedding-enablement` | `adoption` | 로그인 기반 Agent scope와 사람 확인 분리 | Workspace-scoped Agent token은 로그인 Account의 membership·role에서 Agent-safe scope만 파생하고, 사람 전용 command는 브라우저 세션의 fresh preview·단발성 grant와 현재 capability를 별도로 검증한다. | AE-01B | AE-03 | No |
